...
namespace JT.Infrastructure
{
    ...
    {
        /// <summary>
        /// download the images from content's urls to local[default: ~/upload/yyyyMMdd/]
        /// </summary>
        /// <param name="htmlText">url, or html with images</param>
        /// <param name="isHTML"></param>
        /// <param name="path">relative directory to save the image</param>
        /// <returns></returns>
        public static Dictionary<string, string> DownImageByHtmlContent(string htmlText, string path = null, bool isThrowException = false)
        {
            var result = new Dictionary<string, string>();
            if (string.IsNullOrEmpty(path))
                path = "downloads/" + DateTime.Now.ToString("yyyyMMdd");

            string newUrl = string.Empty;

            var imgUrls = HtmlExtension.GetImageUrlsFromHtml(htmlText);
            foreach (var url in imgUrls)
            {
                string fileName = DownImageByUrl(url, path, null, isThrowException);
                result.Add(url, string.Format("{0}//{1}", path, fileName));
            }
        ...
        public static string DownImageByUrl(string url, string savePath, string saveFileName = null, bool isThrowException = true)
        {
            try
            {
                var imgRequst = WebRequest.Create(url);
                using (Stream stream = imgRequst.GetResponse().GetResponseStream())
                {
                    using (Image downImage = Image.FromStream(stream))
                    {
                        string extension = Path.GetExtension(url);
                        if (string.IsNullOrEmpty(extension))
                            extension = GetImageExtension(downImage);

                        if (string.IsNullOrEmpty(saveFileName))
                            saveFileName = Guid.NewGuid().ToString().Replace("-", ""); // don't use dateTime, avoid to get the same fileName

                        var directory = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, savePath);
                        if (!Directory.Exists(directory))
                            Directory.CreateDirectory(directory);

                        downImage.Save(string.Format("{0}\\{1}{2}", directory, saveFileName, extension));
        ...
        /// Make Thumbnail
        /// </summary>
        /// <param name="original">can be url(start with 'http') or absolute path</param>
        /// <param name="savePath"></param>
        /// <param name="width"></param>
        /// <param name="height"></param>
        /// <param name="mode"></param>
        public static void MakeThumbnail(string original, string savePath, int width, int height, ThumbnailMode mode)
        {
            Image originalImage;
            if (original.StartsWith("http", true, System.Globalization.CultureInfo.CurrentCulture))
            {
                var imgRequst = WebRequest.Create(original);
                originalImage = Image.FromStream(imgRequst.GetResponse().GetResponseStream());
            }
            else
            {
                originalImage = Image.FromFile(original);
            }
